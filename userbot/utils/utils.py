...
async def autobot():
    ...
    await bot.send_message(
        BOTLOG_CHATID, "**Sedang membuat bot telegram di @BotFather**"
    )
    who = await bot.get_me()
    name = who.first_name + " Assistance Bot"
    if who.username:
        username = who.username + "_ubot"
    else:
        username = "Zpy" + (str(who.id))[5:] + "sbot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "Zpy" + (str(who.id))[6:] + str(ran) + "sbot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
