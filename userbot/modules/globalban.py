...
async def gban(event):
    ...
    if user.id == (await event.client.get_me()).id:
        await gbun.edit("**Tidak Bisa Gban Akun Sendiri**")
        return
    if user.id in DEVS:
...
async def ungban(event):
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        await ungbun.edit(
            f"**Si** [Mantan](tg://user?id={user.id}) **ini tidak ada dalam daftar gban Anda**"
        )
        return
    san = []
    san = await admin_groups(event)
    ...
    if fiz == 0:
        ...
        return
    await ungbun.edit(
        f"**initiating ungban of the** [Mantan](tg://user?id={user.id}) **in** `{len(san)}` **groups**"
    )
    for i in range(fiz):
        try:
